...
from illixr.analysis.analyze_trials import analyze_trials_projection, combine, path_metric_names, conditions2label, group, important_path_signatures
from illixr.analysis.util import chunker, write_dir, right_pad, summary_stats, histogram, omit, second
import charmonium.time_block as ch_time_block
from charmonium.cache import memoize

@memoize(verbose=False, group=group)
def compare_compute_times(output_dir: Path, proj_compute_times: Mapping[float, Mapping[str, Mapping[str, List[np.ndarray]]]], **kwargs) -> Mapping[str, Any]:
    write_dir({
...
def compare_path_metrics(output_dir: Path, proj_path_metrics: Mapping[frozendict, Mapping[str, Mapping[str, List[np.ndarray]]]], **kwargs) -> Mapping[str, Any]:
    ...
    def inner_summarize_diff(conditions1, conditions2, name, metric):
        ...
        conditions1 = [conditions for conditions in proj_path_metrics.keys() if conditions1 == conditions][0]
    ...
    def summarize_diff(scheduler1, scheduler2):
        conditions1 = {frozendict(omit(condition, {"scheduler", "swap"})) for condition in scheduler_conditions[scheduler1]}
        conditions2 = {frozendict(omit(condition, {"scheduler", "swap"})) for condition in scheduler_conditions[scheduler2]}
    ...
    write_dir({
        output_dir / "path_metrics": {
            ...
            "comparison.txt": "\n".join([
                ...
                for scheduler1, scheduler2 in itertools.product(scheduler_conditions.keys(), repeat=2)
            ]),
            "hist": {
                conditions2label(conditions): {
                    name: {
...
def print_trials(output_dir: Path, condition_trials: Mapping[frozendict, List[Path]], **kwargs) -> Mapping[str, Any]:
    write_dir({
...
def gen_aggregate(metrics_dirs: List[Path], chunk_size: int) -> Mapping[str, Any]:
    return combine(tqdm(
        (
            ...
            for metrics_group in chunker(sorted(metrics_dirs, key=lambda path: path.stat().st_mtime), chunk_size)
        ),
